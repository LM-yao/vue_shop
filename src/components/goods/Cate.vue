<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 表格 :selection-type="false"去除复选框 :expand-type="false"去除展开行 show-index显示索引列 index-text="#"设置数据索引名称 border纵向边框线 :show-row-hover="false"鼠标放上去是否有高亮-->
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        <!-- 是否有效 使用模板列 slot-scope属性接收数据-->
...
export default {
  data() {
    return {
      ...
      columns: [
        ...
        {
          // 作用域插槽，先将type设为template，然后将template设置成具体的作用域插槽的名字
          label: '是否有效',
          // 表示，将当前列定义为模板列
          type: 'template',
          // 表示当前这一列使用模板名称
  ...
  methods: {
    ...
    // 点击按钮，添加新的分类
    addCate() {
      // 再添加之前先进行表单的预验证，拿到引用调用回调函数，用valid接收
